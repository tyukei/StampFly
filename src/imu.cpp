...
void imu_init(void)
{
  ...
  Serial.printf("Start IMU Initialize!\n\r");
  ...
  delay(5);
  Serial.printf("SPI Initilize status:%d\n\r",spi_init());

  //BMI270 Init
  bmi270_dev_init();  
  st = bmi270_init(pBmi270);
  Serial.printf("#INIT Status:%d\n\r", st);
  if (st!=0)
  {
    Serial.printf("BMI270 INIT Fail!\n\r");
    while(1);
  }
  Serial.printf("#Chip ID DEV:%02X\n\r", Bmi270.chip_id);
  Serial.printf("#APP_STATUS:%02X\n\r", Bmi270.aps_status);
  
  Serial.printf("#INIT_STATUS Read:%d\n\r",bmi2_get_regs(0x21, &data, 1, pBmi270));  
  Serial.printf("#INIT_STATUS:%02X\n\r", data);
  //IMU Config
  Serial.printf("#Config Status:%d\n\r", set_accel_gyro_config(pBmi270));
  uint8_t sensor_list[2] = { BMI2_ACCEL, BMI2_GYRO };
  Serial.printf("#Sensor enable Status:%d\n\r", bmi2_sensor_enable(sensor_list, 2, pBmi270));
}
...
void imu_test(void)
{
  ...
  while(1)
  {
    ...
    #if 1
    Serial.printf("%8.4f %7.5f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %d\n\r", 
      (float)(now-st)*1.0e-6,
